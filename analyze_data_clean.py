...
def clean_data(df):
    ...
    # 3. Preserve the 'Lead Source' column as text
    lead_source_col = None
    if 'Lead Source' in df_clean.columns:
        lead_source_col = df_clean['Lead Source'].copy()
    
    # 4. Convert numeric columns (excluding 'Lead Source' if it exists)
    for col in df_clean.select_dtypes(include=['object']).columns:
        if col == 'Lead Source':
            continue  # Skip the Lead Source column
        try:
    ...
    # 5. Put back the original 'Lead Source' column if it existed
    if lead_source_col is not None:
        df_clean['Lead Source'] = lead_source_col
